package com.heye.crm.server.service.impl;

import com.heye.crm.common.model.HyConsume;
import com.heye.crm.server.core.AbstractService;
import com.heye.crm.server.dao.HyConsumeMapper;
import com.heye.crm.server.service.HyConsumeService;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import javax.annotation.Resource;

/**
 * @author : lishuming
 */
@Service
@Transactional
public class HyConsumeServiceImpl extends AbstractService<HyConsume> implements HyConsumeService  {
    @Resource
    private HyConsumeMapper hyConsumeMapper;
}
